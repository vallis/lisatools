...
__version__='$Id: makeTDIsignal-synthlisa2.py 1204 2012-03-09 18:45:53Z m.vallis $'
...
parser = OptionParser(usage="usage: %prog [options] IMMEDIATE.xml/BARYCENTRIC.xml OUTPUT.xml",
                      version="$Id: makeTDIsignal-synthlisa2.py 1204 2012-03-09 18:45:53Z m.vallis $")
...
if options.armlength == 'Standard':
    ...
    if options.LISAmodel == 'Eccentric':
        lisa = synthlisa.EccentricInclined(L,0.0,1.5*pi,-1,0)
    elif options.LISAmodel == 'Static':
        lisa = synthlisa.OriginalLISA(L,L,L)
    elif options.LISAmodel == 'Rigid':
        lisa = synthlisa.CircularRotating(L,0.0,1.5*pi,-1,0)
    else:
...
if options.debugSNR or hasattr(allsources[0],'RequestSN'):
    ...
    if options.noiseOnly:
        Sn = Sx
	print "Stas: no galactic noise added"
    else:
        Sn = Sx + Sgal
    
   
    #fout = open("noiseTest.dat", 'w')
    #for i in xrange(len(fr)):
    #	 fout.write(str(fr[i]) + "     " + str(Sn[i]) + "    " + str(Sgal[i]) + "\n")
    ...
    #### Stas: we use eLISA: we chose X to be the data set
    #SNR = max(snX,snY,snZ)
    SNR = snX
    print "Stas: debugging SNx,y,z =", snX, snY, snZ
...
lisa.Armlength = 3.33; lisa.Armlength_Unit       = 'Second'
