...
: GW()
{
	...
	chi1 = 0.76622880068;
	chi2 = 0.357815817167;
	PolarAngleOfSpin1 = acos(-0.158856838338);
	PolarAngleOfSpin2 = acos(-0.482166499414);
	AzimuthalAngleOfSpin1 = 3.21243381724;
	AzimuthalAngleOfSpin2 = 2.5442762228;
	phic = 3.1710410372;
	InitialPolarAngleL = 2.02230588043;
	InitialAzimuthalAngleL = 4.74885787067;
	TaperApplied = 7.0;
	AmplPNorder = 0.0;
	Tobs = 62914560.0;
	Toffset = -900.0;
...
: GW(Beta_n, Lambda_n, 0.0)
{
	setMass1(Mass1_n);
	setMass2(Mass2_n);
	tc = CoalescenceTime_n;
	setDistance(Distance_n) ; // kpc
	chi1 = Spin1_n;
	chi2 = Spin2_n;
	InitialPolarAngleL = InitialPolarAngleL_n;
	PolarAngleOfSpin1 = PolarAngleOfSpin1_n;
	PolarAngleOfSpin2 = PolarAngleOfSpin2_n;
	InitialAzimuthalAngleL = InitialAzimuthalAngleL_n;
	AzimuthalAngleOfSpin1 = AzimuthalAngleOfSpin1_n;
	AzimuthalAngleOfSpin2 = AzimuthalAngleOfSpin2_n;
	phic = PhaseAtCoalescence_n;
...
{	
	...
	// Allocate (or reallocate) data at the good length
	muly2 = (double*)malloc(idxm*sizeof(double));
	sphily2 = (double*)malloc(idxm*sizeof(double));
	cphily2 = (double*)malloc(idxm*sizeof(double));
	betay2 = (double*)malloc(idxm*sizeof(double));
	sigmay2 = (double*)malloc(idxm*sizeof(double));
	thomasy2 = (double*)malloc(idxm*sizeof(double));
...
void GWFastSpinBBH::rkckstep(double outputvals[], double fourthorderoutputvals[], double *outputthomas, double h, double currentvals[], double currentthomas, double t, double m1, double m2, double Mtot, double Mchirp, double mu, double eta, double chi1, double chi2, double N[], double tc)
{
	...
	k = (double**) malloc(9*sizeof(double*));
	for(i = 0; i < 9; i++)
	...
	for(i = 0; i < 9; i++) 
		intvals[i] = currentvals[i] + B21*k[i][0];
