...
class XSILobject(XMLobject,list):
    ...
    parameters are stored as named attributes (say, XSILobj.Frequency),
    the comment in XSILobj.Comment, the Array/Table in XSILobj.Array and/or
    ...
    (A reference to the first object of each kind is also stored as an attribute
    named by Type (e.g., XSILobj.SourceData or XSILobj.TimeSeries); however,
    this reference is for convenience only, and it will not be updated if and
    when list content is replaced or modified.)
    """
    ...
    def makeXSIL(node,xmlfile=None,self=None):
        """Create an XSILobject from a pyRXP-processed XML tree. This amounts to:
        
        - Constructing an XSILobject of the right type. Currently special cases
        include Source ('PlaneWave'), SampledSource ('SampledPlaneWave'),
        SourceTable ('PlaneWaveTable'), Observable ('TDIObservable'), and
        TimeSeries.
        ...
        - Processing <Array> and <Table> elements if defined inside this <XSIL>
        element. Note that only one of each is allowed; if given, the two are
        accessible as XSILobj.Array and XSILobj.Table.
        
        - Processing any XSIL children, which are stored in the list content
        of the XSILobject (meaning that the Python list interface can be used
        to access, modify, and remove them), and also in attributes
        ...
        # create an object of the correct kind or use what's passed...
        
        if self != None:
            pass
        elif objtype == 'PlaneWave':
            # need the SourceType to allocate the right kind of Source object
            for node2 in node:
            ...
            if 'Binary' in self.Encoding:
                ...
                if type(self.Data) == numpy.ndarray:
                    ...
                    if os.path.abspath(self.Data) != os.path.abspath(filename):
                        ...
                        if os.stat(self.Data)[stat.ST_SIZE] < 50000000:
...
class SampledSource(Source):
    def __init__(self,sourcetype='',name=''):
        super(SampledSource,self).__init__(sourcetype,name)
        
        # override xmltype/Type
