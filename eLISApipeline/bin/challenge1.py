...
#challengestring = "1.1a,1.1b,1.2,2.1"
challengestring = "1.1a"#,1.1b,1.2,2.1"#,1.2,2.1"
...
print "Stas: ", subchallenges
print "Stas, runonly:", runonly

for subchallenge in subchallenges:
    seeds, seedt = twoseeds()
    if subchallenge in runonly:
        duration = 31457280
	print "Stas: subchallenge:", subchallenge 
        
        if not options.trainingOnly:
            run('%(mydir)s/challenge.py -P %(nproc)s            --duration=%(duration)s --seed=%(seeds)s  challenge1.%(subchallenge)s' % globals())
        if not options.blindOnly:   
            run('%(mydir)s/challenge.py -P %(nproc)s --training --duration=%(duration)s --seed=%(seedt)s  challenge1.%(subchallenge)s' % globals())
	    #sys.exit(0)
