	...
	// All the next variables represents a time parameter.*/ 
		...
		*/
		...
		/*! \brief  Order for time propagation computing.
			...
			* \arg 1  : time propagation is computed using special relativity.
			* \arg 2  : time propagation is computed using general relativity.
			* If the satellites are fixed, only \f$ Order=0 \f$ is valid.
			*/
		int OrbOrder;
		/*!\brief
			* Satellites motion.
...
public:
		...
		/*! \brief It returns the value of #OrbMove attribute.*/
		int getOrbMove() {return(OrbMove);};
		...
		bool FindTDIName(const char * generatorname, vector<int> & tmp_TDIPacks);
};
