...
int main(int argc,char **argv)
{
  ...
  double f, theta, phi, A, iota, psi, phase;
  char Gfile[50];
  double *params;
    ...
    while ( !feof(Infile) )
     {
       ...
       if (cnt == ((cnt/100000)*100000)) printf("%ld sources simulated\n", cnt);

       fonfs = f/fstar;

       q = (long)(f*T);

       Sn = (4.0*Ss21+2.0*Ss21*fonfs*fonfs+8.0*Sa21*(1.0+pow((1.0e-4/f),2.0) )*pow(2.0*pi*f,-4.0))/(4.0 * L*L);

       BW = 2.0*(2*pi*f*AU/c+5.0);

       /* compare the aamplitude to the noise in a bin
