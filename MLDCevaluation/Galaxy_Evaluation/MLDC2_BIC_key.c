...
int main(int argc, char* argv[])
{
  ...
  {
    SMAX++;
    fscanf(sfile,"%lf %lf %lf %lf %lf %lf %lf",&f, &theta, &phi, &Amp, &psi, &iota, &phase);
  }
  rewind(sfile);
  SMAX--;  
  ...
  for(i=1; i<=SMAX; i++)
  {
    fscanf(sfile,"%lf %lf %lf %lf %lf %lf %lf",&f, &theta, &phi, &Amp, &iota, &psi, &phase);
    bright[i][0] = 1.0;
    bright[i][1] = f;
  ...
  for(n=1; n<=NW; n++)
  {
    ...
    while(trigger == 0)
    {
      ...
      while(check == 0)
      {
        ...
        if(source<=SMAX)
        {
          if(bright[source][0] > 0.0) check = 1;
        }else check = 1;
      }

      if(source<=SMAX)
      {
        for(i=1; i<=NP; i++) params[i] = bright[source][i];
  ...
  for(n=1; n<=SMAX; n++)
  {
    ...
    if(bright[n][0] > 0.0)
    {
      for(i=1; i<=NP; i++) fprintf(outfile1,"%.12g ",params[i]);
      fprintf(outfile1,"\n");
    
      /* output in focus windows specifically  */
      if(f>=0.2985e-3 && f<=0.3015e-3)
      {  
        for(i=1; i<=NP; i++) fprintf(bfile1,"%.12g ",params[i]);
        fprintf(bfile1,"\n");
      }
      if(f>=0.9985e-3 && f<=1.0015e-3)
      {  
        for(i=1; i<=NP; i++) fprintf(bfile2,"%.12g ",params[i]);
        fprintf(bfile2,"\n");
      }
      if(f>=2.9985e-3 && f<=3.0015e-3)
      {  
        for(i=1; i<=NP; i++) fprintf(bfile3,"%.12g ",params[i]);
        fprintf(bfile3,"\n");
      }
      if(f>=5.9985e-3 && f<=6.0015e-3)
      {  
        for(i=1; i<=NP; i++) fprintf(bfile4,"%.12g ",params[i]);
        fprintf(bfile4,"\n");
      }    
    
    }else
    {
      for(i=1; i<=NP; i++) fprintf(outfile2,"%.12g ",params[i]);
      fprintf(outfile2,"\n");

      /* output in focus windows specifically  */
      if(f>=0.2985e-3 && f<=0.3015e-3)
      {  
        for(i=1; i<=NP; i++) fprintf(rfile1,"%.12g ",params[i]);
        fprintf(rfile1,"\n");
      }
      if(f>=0.9985e-3 && f<=1.0015e-3)
      {  
        for(i=1; i<=NP; i++) fprintf(rfile2,"%.12g ",params[i]);
        fprintf(rfile2,"\n");
      }
      if(f>=2.9985e-3 && f<=3.0015e-3)
      {  
        for(i=1; i<=NP; i++) fprintf(rfile3,"%.12g ",params[i]);
        fprintf(rfile3,"\n");
      }
      if(f>=5.9985e-3 && f<=6.0015e-3)
      {  
        for(i=1; i<=NP; i++) fprintf(rfile4,"%.12g ",params[i]);
        fprintf(rfile4,"\n");
      }    
