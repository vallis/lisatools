...
# choose between 30 and 50 sources

howmany = random.randint(30,50)
chosensources = []
...
for source in chosensources:
    ...
    run('%(mydir)s/makesource-GalacticBinary.py --seed=%(binaryseed)s --requestSN=%(snr)s --latitude=%(lat)s --longitude=%(lon)s --centerf=%(freq)s --deltaf=0.0 --sourceName="GB #%(swritten)s" Source/GB1.1.5-%(swritten)s.xml' % globals())
