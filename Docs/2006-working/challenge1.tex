...
\lambda^2 & t_c        & \text {instance of coalescence}\\
...
after time $t = t(R_{taper})$, time when radius of orbit is 
$R_{taper}$.
\be
w(t) = \frac1{2}\left( 1 - \tanh\left[A(x(t) - x_{max})\right]\right),
\en
where $x$ is defined by eqn. (\ref{x}) and for $x_{max}$ we use lowest order approximation:
$$
x_{max} = \frac{M}{R_{taper}} 
$$
and coefficient $A = 150$ was found empirically to produce smooth damping 
for $R$ in range $[6M, 8M]$. 
%For more details on implementation of
%the taper see next subsection.
...
\item {\tt BBHChallenge1(float mass1, float mass2)} constructor,
the input are two masses in units of solar mass.

\item {\tt SetInitialOrbit(float coalTime, float phi0)}, defines initial 
orbit, initial orbital angular frequency $\omega_0$ can be found from 
coalescence time (``{\tt coalTime}''), $t_c$, using eqn.~(\ref{fr}).
Second parameter defines initial orbital phase.

\item {\tt ComputeInspiral(float t0, float timeStep, float maxDuration)}:
this method computes orbital evolution. Method {\tt SetInitialOrbit}
should be called beforehand. It takes initial time {\tt t0} (could be 
negative), {\tt timeStep} of evolution (cadence), and maximal duration
{\tt maxDuration}. The last parameter is used in the challenge 1.2.2, where
the coalescence occurs outside the time of observation.
...
\item {\tt ComputeWaveform(float truncateTime, float taper,  std::vector<double>\& hPlus,\\
                             std::vector<double>\& hCross)}:
 This method computes waveform in the
...
this method is the same as above, but computes waveform in the SSB frame according to
eqn.(\ref{hps}, \ref{hcs}).

\item {\tt double EstimateTc(float omega0)} and {\tt double EstimateFreq0(float tc)} 
are two auxiliary methods used to compute time of coalescence for given 
initial orbital angular frequency and inverse, computing initial orbital angular 
frequency for a given coalescence time. 
...
%As promised we should give some more details of the implementation 
%of the taper. 
%\begin{enumerate}
%\item first we need to find orbital angular frequency $\omega_{R_t}$ which 
%corresponds the orbital separation $r = R_t$, where $R_t$ radius defined by 
%parameter $\tt taper$ and is taken to be $8M$ for the first challenge.
%Here we use the lowest order relation between orbital radius and
%frequency $M/r \approx \left(M\omega\right)^{2/3}$.
%\item next is constructing and applying the taper. Isn the method
%{\tt ComputeInspiral} we have computed angular orbital frequency and phase
%sampled at the instances of time defined by cadence. Due to this sampling we 
%project $\omega_{R_t}$ to the nearest discrete value.
%Then we use sampled version of the eqn.~(\ref{wnd}) and multiply 
%it with sampled waveform. 

%\item all together is something like that:

%{\tt -loop over angular orbital frequencies (sampled with cadence dt)\\
%     -if we need to apply taper and if $1/(M\omega_k)^{2/3} \le taper$, then
%     remember $ind = k$ and break \\
%     -define $tk = 0.5*(size + ind)$ where $size$ is the size of the waveform\\
%     -loop over orbital phase (sampled with cadence dt)\\
%     -compute sampled taper $ wk = 0.5( 1.0 + tanh(0.1\left(7./taper\right)^{7.5}(tk-k)))$\\
%     -apply the taper $hp(k)*wk$
%     }     
%\end{enumerate}
...
& & \left. \left( \frac{784676435}{3048192} + 
\frac{5429}{504}\eta + \frac{617}{72}\eta^2\right) (\pi Mf)^{4/3}
\right\}
\ena
...
\psi_{SL} = arctan\bigl(\frac{\sin\theta_L \sin(\phi_S-\phi_L)}{\cos \theta_S \sin \theta_L \cos(\phi_S - \phi_L) - \cos\theta_L \sin\theta_S}\bigr) \,.
\end{equation}
...
\item {\tt EvolveOrbit(float t0, float nu0, float eccen, float gamma0, \
...
$= \lambda$. NOTE that initial conditions are specified at $t=0$
which corresponds to the beginning of the observed time. The first 
parameter {\tt t0} is usually taken to be less than zero (required for TDI),
therefore orbit is evolved backwards to the time {\tt t0} and then forward
from the instance $t=0$. 
