...
int main(int argc,char **argv) {
  ...
  if (argc < 2) 
    KILL("A data set name was not entered at the command line prompt.\n");
 ...
 /* convert Numerical Recipes FFT output into a strain spectral density */
  ...
  outfile = fopen("X_FFT.dat", "w");

  for(i=imin; i < imax; i++)
   {
     j = 2*i;
    fprintf(outfile,"%e %e\n", log10(((double)i)/T), 0.5*log10(2.0*(XTDI[j]*XTDI[j]+XTDI[j+1]*XTDI[j+1])));
   }
  ...
  outfile = fopen("Y_FFT.dat", "w");

  for(i=imin; i < imax; i++)
   {
     j = 2*i;
    fprintf(outfile,"%e %e\n", log10(((double)i)/T), 0.5*log10(2.0*(YTDI[j]*YTDI[j]+YTDI[j+1]*YTDI[j+1])));
   }
  ...
  outfile = fopen("Z_FFT.dat", "w");

  for(i=imin; i < imax; i++)
   {
     j = 2*i;
    fprintf(outfile,"%e %e\n", log10(((double)i)/T), 0.5*log10(2.0*(ZTDI[j]*ZTDI[j]+ZTDI[j+1]*ZTDI[j+1])));
   }
