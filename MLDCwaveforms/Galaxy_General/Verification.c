...
int main(int argc,char **argv)
{
  ...
  double SNRX, SNRA, SNRE, SNR;
  double SAE, SXYZ;
  double SNR_av, SNRX_av, x;
  double SNR_max, SNRX_max, SNR_min, SNRX_min;
  long M, N, q;
  long rSeed;
  long i, j, k, cnt, mult, ii, Nav;
  double f, fdot, A, theta, phi, iota, psi, phase;
  FILE* vfile;
  FILE* nfile;
  FILE* sky;
  ...
  if(argc !=3) KILL("Verification verification.dat confusion_noise.dat\n");

  vfile = fopen(argv[1],"r");
  nfile = fopen(argv[2],"r");
  sky = fopen("Data/sky_16.dat","r");
  ...
  if((T/year) <= 1.0) mult = 1;
    ...
    while ( !feof(vfile) )
     {
       ...
       /* average over inclination and polarization angle */
      for(ii=1; ii<=Nav; ii++)
       {
	...
	cnt++;
      ...
      for(i=1; i<=2*M; i++)
       {
	 // A channel
	 XSL[i] = (2.0*XLS[i] - YLS[i] - ZLS[i])/3.0;
         // E channel
         YSL[i] = (ZLS[i]-YLS[i])/sq3;
       }
	 ...
	 SNRA += 4.0*(XSL[2*i-1]*XSL[2*i-1]+XSL[2*i]*XSL[2*i]);
	 SNRE += 4.0*(YSL[2*i-1]*YSL[2*i-1]+YSL[2*i]*YSL[2*i]);
       }
      ...
      if(SNRX > SNRX_max) SNRX_max = SNRX;
      if(SNR < SNR_min) SNR_min = SNR;
      if(SNRX < SNRX_min) SNRX_min = SNRX;
      ...
      printf("SNRX (%f,%f,%f) SNR_AE (%f,%f,%f)\n", SNRX_min, SNRX_av, SNRX_max,
             SNR_min, SNR_av, SNR_max);
