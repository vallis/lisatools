...
__version__ = '$Id: challenge.py 1109 2009-10-29 16:39:42Z lisa.apc $'
...
def run(command,quiet = False):
    ...
    print "Stas:", outfilename, errfilename
    if not quiet:
...
parser = OptionParser(usage="usage: %prog [options] CHALLENGENAME...",
                      version="$Id: challenge.py 1109 2009-10-29 16:39:42Z lisa.apc $")
...
parser.add_option("-N","--noNoise",
                  action="store_true", dest="nonoise", default=True,
                  help="do not include noise [default True]")
...
parser.add_option("-F", "--nohighfreq",
                  action="store_true", dest="nohighfreq", default=True,
                  help="do not generate high-frequency datasets [do not generate by default]")
...
### Stas: I have left it as it is, since it does check for name challenge1
...
### Stas I don't do LISA simulator and LISACode, so far I'll run only synthetic LISA
...
if (not makemode) and (not sourcekeyfile):
	...
	#print "=====> ",  'python %s %s %s -P %s' % (sourcescript,seed, trainflag,options.nproc) 
	#print istraining, options.nproc
        run('python %s %s %s %s' % (sourcescript,seed, trainflag ,options.nproc))
...
#### Stas: I am running for eLISA, so I should use makeTDIsignal-synthlisa2, where I can specify the armlength
    ...
    for xmlfile in glob.glob('Barycentric/*-barycentric.xml') + glob.glob('Immediate/*.xml'):
        ...
        if (not makemode) or newer(xmlfile,tdifile):
            ...
            if 'challenge4' in challengename and 'Burst' in xmlfile:
                prun('%(execdir)s/makeTDIsignal-synthlisa2.py --armlength=3.33 %(runoptions)s --duration=%(duration)s --timeStep=1.875 %(xmlfile)s %(tdifile)s')
            else:
                prun('%(execdir)s/%(runfile)s %(runoptions)s --armlength=3.33 --duration=%(duration)s --timeStep=%(timestep)s -D %(xmlfile)s %(tdifile)s')

#### Stas: I do not make noise so far, it will/should be done by Antoine separately
...
if dolisacode2 :
    ...
    ### Generate signal only data
    
    for xmlfile in glob.glob('Barycentric/*-barycentric.xml') + glob.glob('Immediate/*.xml'):
        if 'barycentric.xml' in xmlfile:
            tdifile = 'TDI/' + re.sub('barycentric\.xml$','tdi-freq-lisacode2',os.path.basename(xmlfile))
        else:
            tdifile = 'TDI/' + re.sub('\.xml$','tdi-freq-lisacode2',os.path.basename(xmlfile))

        if (not makemode) or newer(xmlfile,tdifile):
            prun('%(execdir)s/%(runfile)s %(runoptions)s --NoNoise --duration=%(duration)s --timeStep=%(timestep)s %(xmlfile)s %(tdifile)s')


    ### Generate noise only data

    noisefile = 'TDI/tdi-freq-lisacode2-noise-only'
    prun('%(execdir)s/%(runfile)s --seed=%(seednoise)s --duration=%(duration)s --timeStep=%(timestep)s %(noiseoptions)s %(noisefile)s')
    ...
    for xmlfile in glob.glob('Barycentric/*-barycentric.xml') + glob.glob('Immediate/*.xml'):
        if 'barycentric.xml' in xmlfile:
            tdifile = 'TDI/' + re.sub('barycentric\.xml$','tdi-freq-lisacode2-noise',os.path.basename(xmlfile))
        else:
            tdifile = 'TDI/' + re.sub('\.xml$','tdi-freq-lisacode2-noise',os.path.basename(xmlfile))
