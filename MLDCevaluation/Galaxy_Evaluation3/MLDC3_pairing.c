...
int main(int argc, char* argv[])
{
  ...
  if(argc!=4) KILL("to run:  ./MLDC3_pairing keyfile sourcefile nametag\n");
  ...
  {
    NR++;
    fscanf(sfile,"%lf %lf %lf %lf %lf %lf %lf %lf",&f, &fdot, &theta, &phi, &Amp, &iota, &psi, &phase);
    q = (long)floor(f*T);
    if(q > qmax) qmax = q;
  }
  rewind(sfile);
  ...
  for(i=1; i<=NI; i++)
  {
    ...
    if(count[q] > npbin) 
    {
      printf("bin %i is multiply populated\n", q);
      n++;
    }
  else 
    {
  ...
  printf("total number of multiply populated bins: %i \n", n);
  ...
  fprintf(outfile, "| # rec | # inj | nwr | correlation | SNR rec | SNR inj | d_f*T |  d_fdot*T*T | d_theta | d_phi | d_A/A | d_iota | d_psi | d_phase |\n");
  ...
  for(n=1; n<=NR; n++)
  {
      ...
      if(corr[i_match][j_match] < 0.5)
	{
         ...
         fprintf(sfile,"%.12lg ",rparams[n][8]);
    ...
    if(source_match > 0)
    {
      ...
      del[2] = -(iparams[source_match][8] - rparams[n][8])*T*T;
      del[3] = -(iparams[source_match][2] - rparams[n][2]);
      del[4] = -(iparams[source_match][3] - rparams[n][3]);
      del[5] = -(iparams[source_match][4] - rparams[n][4])/iparams[source_match][4];
      del[6] = -(iparams[source_match][5] - rparams[n][5]);
      del[7] = -(iparams[source_match][6] - rparams[n][6]);
      del[8] = -(iparams[source_match][7] - rparams[n][7]);
