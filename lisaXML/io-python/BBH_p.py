...
samples = 2**21 + 2*int(prebuffer/stime)

(hp,hc) = transpose(getobsc(samples,stime,(mysystem.hp,mysystem.hc),inittime))
...
sampledwave = SampledWave(hpp,hcp,samples,stime,prebuffer,
                          1.0,None,2,
                          mysystem.initargs[-3],
                          mysystem.initargs[-2],
                          0.0)
