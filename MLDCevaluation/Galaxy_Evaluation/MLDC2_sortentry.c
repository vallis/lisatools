/*
 gcc MLDC2_sortentry.c nrutil2.c -lm -o MLDC2_sortentry
 */
/***************************  REQUIRED LIBRARIES  ***************************/
...
int main(int argc, char* argv[])
{
  ...
  if(argc!=2) KILL("to run:  ./MLDC2_sortentry entryfile\n");
  ...
  {
    NI++;
    fscanf(ifile,"%lg %lf %lf %lf %lf %lf %lf %lf",&x, &f, &theta, &phi, &Amp, &psi, &iota, &phase);
  }
  rewind(ifile);
  NI--;    
  ...
  for(i=1; i<=NI; i++)
  {
    ...
    q = name[indx[i]];
    fprintf(bfile1,"%ld  %.12lg %.10lg %.10lg %.10lg %.10lg %.10lg %.10lg\n", q, f, theta, phi, Amp, psi, iota, phase);
  }
