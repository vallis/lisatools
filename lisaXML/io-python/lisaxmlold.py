...
class lisaXML(writeXML):
    ...
    def processObject(self,obj,name=None,comments=''):
        ...
        xsildict['Name'] = name and name or obj.name

        params.append( ('Param',{'Name': 'SourceType'},[obj.xmltype]) )
        ...
        for arg in obj.outputlist:
            ...
            if hasattr(obj,arg[0]):
                argvalue = getattr(obj,arg[0])
            else:
                argvalue = convertParam(obj,arg)
            
                if argvalue == None and arg[2] != None:
                    argvalue = arg[2]
                else:
                    raise AttributeError, 'lisaXML.processObject(): missing parameter %s in object %s' % (arg[0],obj)
            ...
            # (note: convertParam and convertUnit return argument with unit)

            if isinstance(argvalue,tuple):
                if argvalue[1] != arg[1]:
                    argvalue = convertUnit(argvalue[0],argvalue[1],arg[1],arg[0])
            else:
    ...
    def SourceData(self,source,name='',comments=''):
        ...
        self.theSourceData.append(self.processObject(source,name,comments))
