...
int main(int argc,char **argv)
{
  ...
  // This has not yet been properly updated for Challenge 3
  ...
  if(vflag == 1)
    ...
    printf("Using bulge-fixed catalogs.\n"); 

      if ((Infile = fopen("Data/dwd_GWR_MLDC_bulgefix.dat","r")) == NULL)
       {
   ...
   fclose(Infile);
      ...
      if ((Infile = fopen("Data/AMCVn_GWR_MLDC_bulgefix_opt.dat","r")) == NULL)
       {
    ...
    while ( !feof(Infile) )
     {
       char buffer[256];
       if(!fgets(buffer,256,Infile)) break; /* read into string */
       if(buffer[0] >= 'A') continue;       /* skip alphabetical comments */
