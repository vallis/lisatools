...
Ce documents d\'ecrit, dans la section~\ref{SCode}, la structure du code, dans la section~\ref{SInstall}, l'installation et dans la section~\ref{SUse}, l'utilisation de LISACode. La section~\ref{SConfigFile} d\'ecrit le fichier de configuration ainsi que les diff\'erents param\`etres qui configurent une simulation. \\
...
\caption{\small Structure du simulateur LISACode. Les boites \textcolor{red}{rouge} repr\'esentent les modules principaux et les boites en \textcolor{mygreen}{vert} les modules d'interface. Le nom g\'en\'erique des modules est en \textcolor{blue}{bleu}.\small} 
...
Un ingr\'edient important de la r\'eponse de LISA et par cons\'equent des codes est la sp\'ecification de bruits de diff\'erente nature. Cela inclut le bruit optique du au \emph{shot noise} aux \'el\'ements d\'ecris dans le tableau \ref{table_error}. Le bruit des masses inertielles et le bruit laser peuvent aussi \^etre d\'efinis en entr\'ee. Normalement, ces bruits sont d\'efinis comme des bruits blancs dans une bande fr\'equentielle d\'efinie mais diff\'erentes formes peuvent \^etre utilis\'ees.
\begin{table}[htdp] 
\caption{Budget des erreurs bas\'e sur le tableau 4.1 du  \cite{LISAPPA}.  
...
\begin{enumerate}
\item T\'el\'echarer le fichier lisacode-1.4.4.tar.gz\\ (sur le web : http://www.apc.univ-paris7.fr/Downloads/lisa/LISACode/ )\\ et d\'eplacer le fichier dans le r\'epertoire ({\it MyDirectory} dans l'exemple) o\`u vous souhaitez installer LISACode.
...
Si le programme s'est install\'e normalement, 3 ex\'ecutables se trouvent dans un r\'epertoire {\it MyDirectory/lisacode-1.4.4/Main/Test}. Les 3 ex\'ecutables sont :
\begin{itemize}
...
Les diff\'erentes options d'installation possibles sont d\'etaill\'ees dans le fichier  {\it INSTALL} situ\'e dans {\it MyDirectory/lisacode-1.4.4}.\\

IMPORTANT : Si \`a la configuration le chemin du r\'epertoire contenant la {\bf fftw3} n'est pas trouv\'e, il faut le sp\'ecifier explicitement en ajoutant les arguments : \\
 {\tt CPPFLAGS=-I/cheminfftw3/include LDFLAGS=-L/cheminfftw3/lib} \\
...
\hphantom{aaaaa}\texttt{./LISACode  MyDirectory/lisacode-1.4.4/Example\_Input/ConfigRefBase 123456}
\end{enumerate}
...
\label{SWindows}
Il faut utiliser Cygwin.

Sinon il existe un ex\'ecutable de LISACode sous Windows qui a \'et\'e test\'e sous Windows XP mais aui n'est pas \`a jour !
...
Il est \'egalement possible de spécifier la racine du générateur de nombre aléatoire (\texttt{RandomSeed}) et des options par la ligne de commande :\\
\hphantom{aaa}\texttt{./LISACode [Options] fichier\_de\_configuration RandomSeed}\\
Les options possibles sont :
\begin{itemize}
\item \texttt{-h} : Ajoute une en-tête dans les fichiers binaires.
\end{itemize}
...
\label{SSConfGen}
Ce fichier contient toutes les information pour configurer le simulateur. Il y a deux types de fichier possibles : ASCII (cf.~\ref{SSConfACII}) ou XML (cf.~\ref{SSConfXML}). 
%Le fichier XML peut \^etre construit avec le logiciel d'interface graphique {\it LISA\_AutoGUI.jar}. 
Les tableaux \ref{table_paramTime} , \ref{table_paramPrecisionTDI}, \ref{table_paramOrbits}, \ref{table_paramDetector}, \ref{table_paramRecord}, \ref{table_paramGW}, \ref{table_paramNoise} , \ref{table_paramUSO} and \ref{table_paramTDI}  listent les param\`etres.
\begin{table}[p] 
...
\begin{figure}[!htb] 
\centering \includegraphics[width=8cm]{Figures/GWParameters2.eps} 
...
\begin{table}[p] 
\caption{Liste des g\'en\'erateurs pr\'ed\'efinis dans LISACode ; type \textbf{TDI}.}
\begin{center} 
...
\hline
X or Xf or X1s1 & Michelson de g\'en\'eration 1.5 centr\'e sur SC1 & 1 , 35 , 364 , 3653 , -4 , -53 , -521 , -5235  \\ 
\hline
Y or Yf or X1s2 & Michelson de g\'en\'eration 1.5 centr\'e sur SC2 & 2 , 16 , 145 , 1461 , -5 , -61 , -632 , -6316  \\ 
\hline
Z or Zf or X1s3 & Michelson de g\'en\'eration 1.5 centr\'e sur SC3 & 3 , 24 , 256 , 2542 , -6 , -42 , -413 , -4124  \\ 
\hline
...
\hline
Alpha or alpha & G\'en\'erateur de  g\'en\'eration 1.5 $\alpha$ & -1 , -32 ,  -133 , 4 , 455 , 56  \\ 
\hline
Beta or beta & G\'en\'erateur de g\'en\'eration 1.5 $\beta$ & -121 , -2 , -13 , 64 , 5 , 566  \\ 
\hline
Gamma or gamma &G\'en\'erateur de g\'en\'eration 1.5 $\gamma$ & -21 , -232 , -3 , 464 , 45 , 6  \\ 
\hline
P1 & G\'en\'erateur Beacon sans faisceau re\c cu sur SC1 & 25 , -63 , -22 , 66 , 642 , -216 , 1463 , -1425  \\ 
\hline
Q1 & G\'en\'erateur Beacon sans faisceau re\c cu sur SC2 & 36 , -41 , -33 , 44 , 453 , -324 , 2541 , -2536  \\ 
\hline
R1 & G\'en\'erateur Beacon sans faisceau re\c cu sur SC3 & 14 , -52 , -11 , 55 , 561 , -135 , 3652 , -3614  \\ 
\hline
...
\begin{table}[p] 
\caption{Suite du tableau~\ref{table_paramTDI}; type \textbf{TDI}.}
\begin{center} 
\begin{tabular}{|p{35.mm}|p{60.mm}|p{65.mm}|}
\hline
Nom & D\'etails & Pack Value (in LISACode) \\
...
\hline
A ou Aa ou AX & G\'en\'erateur \`a bruit non-corr\'el\'e & [X]  $\times -1/\sqrt{2}$ , [Z] $\times1/\sqrt{2}$\\ 
\hline
E ou Ea ou EX & G\'en\'erateur \`a bruit non-corr\'el\'e & [X] $\times 1/\sqrt{6}$ , [Y] $\times -2/\sqrt{6}$ , [Z] $\times 1/\sqrt{6}$\\ 
\hline
T ou Ta ou TX & G\'en\'erateur \`a bruit non-corr\'el\'e & [X] $\times 1/\sqrt{3}$ , [Y] $\times 1/\sqrt{3}$ , [Z] $\times 1/\sqrt{3}$ \\
\hline
Ab ou AMLDC & G\'en\'erateur \`a bruit non-corr\'el\'e & [X]  $\times 2/3$ , [Y] $\times -1/3$ , [Z] $\times -1/3$\\ 
\hline
Eb ou EMLDC & G\'en\'erateur \`a bruit non-corr\'el\'e & [Y] $\times  -1/\sqrt{3}$ , [Z] $\times 1/\sqrt{3}$\\ 
\hline
Tb ou TMLDC & G\'en\'erateur \`a bruit non-corr\'el\'e & [X] $\times -\sqrt{2}/3$ , [Y] $\times -\sqrt{2}/3$ , [Z] $\times -\sqrt{2}/3$ \\
\hline
\end{tabular} 
\end{center} 
\label{table_paramTDI2} 
\end{table}
...
\item { \it Type d'orbites } : Ce param\`etre {\tt Type} permet de sp\'ecifier le type d'orbites utilis\'es : 
\begin{itemize}
\item{ \tt LISACode} : orbites d\'efinies par le laboratoire ARTEMIS~\cite{OrbitLISA}. 
\item{ {\tt MLDC} ou {\tt Pseudo\_LISA} } : orbites d\'efinies  par le Mock LISA Data Challenge~\cite{SyntheticLISA}.
\item{ \tt ESA} : orbites issues d'un calcul de l'ESA prenant en compte des éphémérides pour les autres corps du système solaire (lu dans le fichier 'Orbit\_ESA.input.txt'). 
\end{itemize}
Ligne de configuration pour d\'efinir une simulation utilisant les orbites d\'efinies par le laboratoire ARTEMIS~\cite{OrbitLISA} : \\
\hphantom{aaaaa}\texttt{Orbits : Type LISACode ;}  \\

\item { \it Approximation dans le calcul des orbites } : Ce param\`etre {\tt Approx} permet de sp\'ecifier le type d'orbites utilis\'es : 
\begin{itemize}
\item{ \tt Static} : Satellites fixes. 
\item{ \tt Rigid} : Satellites en mouvement avec une configuration rigide si les orbites du MLDC sont utilis\'es.
\item{ \tt Eccentric} : Satellites en mouvement avec une configuration rigide si les orbites du MLDC sont utilis\'es (identique \`a {\tt Eccentric} si les orbites sont du type { \tt LISACode} ou { \tt ESA}). 
\end{itemize}
Ligne de configuration pour d\'efinir une simulation utilisant les orbites d\'efinies par le laboratoire ARTEMIS~\cite{OrbitLISA} : \\
\hphantom{aaaaa}\texttt{Orbits : Approx Eccentric ;}  \\
...
\item { \it Phasem\`etres interne dans les fichiers de sorties} : Sp\'ecifie si les donn\'ees des phasemetres internes sont dans les fichiers de sorties dans le cas sans bruit. Deux valeurs possibles : {\tt Always}, les donn\'ees sont toujours \'ecrites; {\tt IfNoises}, les donn\'ees sont \'ecrites seulement si les bruits sont consid\'er\'e dans la simulation (puisque dans ce cas ils ne sont pas nulles !). Exemple :
\hphantom{aaaaa}\texttt{Detector : InternalPhasemeters IfNoises ;}  \\
...
\item { \it Onde gravitationnelle d'une binaire spiralante BH-BH avec spin } \texttt{\{FastSpinBBH\}} : Onde gravitationnelle d'une binaire spiralante composée de deux trous noirs avec prise en compte du spin. Le calcul de la précession du plan orbital est réalisée par une intégration avec la méthode Runge-Kutta d'ordre 6. L'amplitude est calculée dans l'approximation Post- Newtonienne avec un ordre pouvant aller jusqu'à 2 PN \texttt{\{PN\}}. On d\'efinit les masses des deux astres $m_{1}$ \texttt{\{M1\}} et $m_{2}$ \texttt{\{M2\}} en masse solaire, le temps de coalescence  $t_{coal}$ \texttt{\{tcoal\}} en secondes, la phase initiale $\phi_{0}$ \texttt{\{phase\}} entre $0$ et $2\pi$ et la distance entre la source et le d\'etecteur $r$ \texttt{\{r\}} en kpc. On spécifie également l'amplitude des spins, \texttt{\{S1, S2\}}, entre 0 et 1, leur angle polaire \texttt{\{polS1, polS2\}} et leur angle azimutal \texttt{\{polS1, polS2\}}. On spécifie également la direction initial du moment orbital par son angle polaire \texttt{\{polL\}} et son angle azimutal \texttt{\{azL\}}. Le calcul de l'évolution temporelle des contraintes gravitationnelles de l'onde dans le référentiel barycentrique (BRF : Barycentric Reference Frame) ${h}_{B+} (t)$ et ${h}_{B\times} (t)$ est basé sur les articles \cite{MLDC3} et \cite{ApostolatosSpin}.  \\
Ligne de configuration pour d\'efinir une onde gravitationnelle issue d'une binaire de trous
 noirs  spiralante avec spin de direction $\beta = 37.0 ^{o}$, $\lambda= 285.9 ^{o}$, de masses $M1 = 1465580.0 \ M_S$ et $M2 = 868710.0 \ M_S$ , de temps de coalescence $3.1e7 \; s$,  de phase initiale $ 1.1 \; rd$, de distance $ 1.0e6 \; kpc$, d'angle polaire moment orbital initial $162.0 ^{o}$ et d'azimut $328.9 ^{o}$, de spin de l'objet 1 d'intensité $0.077 $, d'angle polaire $160.0 ^{o}$, d'azimut $130.6 ^{o}$ et de spin de l'objet 2 d'intensité $0.27 $, d'angle polaire $239.0 ^{o}$, d'azimut $162.0 ^{o}$ (amplitude calculée à l'ordre 2 PN): \\
\hphantom{aaaaa}\texttt{GW : Bet 37.0 , Lam 285.9 , Psi 0.0  : FastSpinBBH : M1 1465580.0 , M2 868710.0 , tcoal 3.1e7 , phase 1.1 , r 1.0e6 , S1 0.077 , S2 0.27 , polS1 160.0 , polS2 130.6 , azS1 0.8 , azS2 239.0 , polL 162.0 , azL 328.9 , PN 2 ;}  

\item { \it Onde gravitationnelle quelconque (lecture d'un fichier)} \texttt{\{File\}} : Onde gravitationnelle quelconque dont les \'evolutions temporelles des composantes de polarisation exprimées dans le référentiel canonique de l'onde ($ h_{S +} \; \; h_{S \times} $) sont lues dans un fichier. Il y a quatre format de fichier possible : \\

\begin{itemize}

\item { \sf Fichier ASCII à 3 colonnes : $ temps, \; h_{S +} \; h_{S \times} $ } :  Ligne de configuration pour lire le fichier ASCII à 3 colonnes {\it GWFile.txt} \footnote{\label{N:GWFileExtension} L'extension du fichier est quelconque excepté {\tt .xml} qui est réservé pour les fichiers XML de données du MLDC} (direction de la source $\{ \beta = 50^{o}$, $\lambda = 230^{o} \} $ et polarisation $\psi = 15^{o}$) : \\
\texttt{GW : Bet 50 , Lam 230 , Psi 15 : File : GWFile.txt ;}  \\

\item { \sf Fichier ASCII à 2 colonnes : $ h_{S +} \; h_{S \times} $ avec le temps initial $t_0$ et le pas de temps $\Delta t$ spécifiés dans le fichier de configuration } :  Ligne de configuration pour lire le fichier ASCII à 2 colonnes {\it GWFile2.txt} \footnote{cf. note~\ref{N:GWFileExtension}} dont le temps initial $t_0 = -900 s$ et le pas de temps $\Delta t = 15 s$ (direction de la source $\{ \beta = 50^{o}$, $\lambda = 230^{o} \} $ et polarisation $\psi = 15^{o}$). \\
\texttt{GW : Bet 50 , Lam 230 , Psi 15 : File : GWFile2.txt ASCII -900 15 ;}  \\

\item { \sf Fichier binaire à 2 colonnes : $ h_{S +} \; h_{S \times} $ avec la longueur $N_D$ (nombre d'enregistrement ou lignes) et le nombre de type de données $N_R$ (\emph{records} ou colonnes), le temps initial $t_0$ et le pas de temps $\Delta t$ spécifiés dans le fichier de configuration } :  Ligne de configuration pour lire le fichier binaire à 2 colonnes {\it GWFile3.bin}\footnote{cf. note~\ref{N:GWFileExtension}} dont la longueur $N_D = 4194304 $, le nombre de colonne $N_R = 2$, le temps initial $t_0 = -900 s$ et le pas de temps $\Delta t = 15 s$ (direction de la source $\{ \beta = 50^{o}$, $\lambda = 230^{o} \} $ et polarisation $\psi = 15^{o}$). \\
\texttt{GW : Bet 50 , Lam 230 , Psi 15 : File : GWFile3.bin BINARY 4194304 2 -900 15 ;}  \\

\item { \sf Fichier binaire à 2 colonnes : $ h_{S +} \; h_{S \times} $ décrit par un fichier XML au format du Mock LISA Data Challenge } :  Ligne de configuration pour lire un fichier binaire à 2 colonnes décrit par le fichier XML {\it GWFile4.xml}\footnote{Pour ce type de fichier l'extention {\tt .xml} est indispensable.}  (direction de la source $\{ \beta = 50^{o}$, $\lambda = 230^{o} \} $ et polarisation $\psi = 15^{o}$). \\
\texttt{GW : Bet 50 , Lam 230 , Psi 15 : File : GWFile4.xml ;}  \\
...
\item { \it Bruit lu dans un fichier }: Mod\'elisation d'un bruit \`a partir des valeurs d'un fichier ( 2 colonnes : temps bruit). Le programme charge l'ensemble des valeurs est commence \`a lire les valeurs à partir d'un point quelconque du fichier. Une fois que la dernière donnée est lue, la lecture recommence à la première donnée.\\
Ligne de configuration pour d\'efinir un bruit lu dans le fichier {\it NoiseOB.txt} contenant des données de variation de fréquence, d'où le facteur multiplicatif de normalisation à $1 / f_{0} = 3.5491 \times 10^{-15}$ pour passer en unités de variation relative de fréquence. Ce bruit est attribu\'e au banc optique du satellite 1 en vis-\`a-vis avec le satellite 2 : \\
\hphantom{aaaaa}\texttt{Noise : Laser 1 0 : File NoiseOB.txt 3.5491e-15;}  \\
\end{itemize}
...
\hphantom{aaaaa}\texttt{TDI : Z2Man  3 , 24 , 256 , 2542 , 25416 , 254142 , 2541413 , 25414124 , -6 , -42 , -413 , -4124 , -41253 , -412524 , -4125256 , -41252542 ;} 
...
\begin{tabular}{|c|c|c|c|}
\hline
Opérateur & Bras & Emetteur $\rightarrow$ R\'ecepteur & Code \\
\hline
...
\end{tabular}
\end{center}
\caption{Tableau donnant la correspondance entre les chiffres du pack et le bras concerné par l'opérateur de retard.}
\label{TDICodeDelay}
\end{table} 

Il est également possible d'associer à chaque pack du générateur, un facteur\footnote{Indispensable par exemple pour d\'efinir A,E et T. \cite{VallisCsqPerteLiens}}. Dans la syntaxe, ce facteur suit le nombre entier décrivant le pack, séparé de celui-ci par un {\tt x}, avec touours un espace entre chaque groupe de caract\`eres. \\
%
+ Ligne de configuration pour d\'efinir le g\'en\'erateur TDI \`a bruit non-corr\'el\'e $E_b$\footnote{Le g\'en\'erateur E est d\'efini comme $E = {Y - Z \over \sqrt{3}}$}  : \\
\hphantom{aaaaa}\texttt{TDI : EMan 2 x 0.577350269 , 16 x 0.577350269 , 145 x 0.577350269 ,}\\
\hphantom{aaaaa}\texttt{1461  x 0.577350269 , -5 x 0.577350269 , -61 x 0.577350269 , -632}\\
\hphantom{aaaaa}\texttt{x 0.577350269 , -6316 x 0.577350269 , 3 x -0.577350269 , 24 x }\\
\hphantom{aaaaa}\texttt{-0.577350269 , 256 x -0.577350269 , 2542 x -0.577350269 , -6 x }\\
\hphantom{aaaaa}\texttt{-0.577350269 , -42 x -0.577350269 , -413 x -0.577350269 , }\\
\hphantom{aaaaa}\texttt{-4124 x -0.577350269  ; } \\


Pour un certain nombre de générateurs, l'ensemble de packs est predefinis dans LISACode. Pour ces generateurs listes dans les tableaux~\ref{table_paramTDI} et~\ref{table_paramTDI2},  le nom du generateur suffi. \\
%
+ Ligne de configuration pour d\'efinir le g\'en\'erateur TDI Michelson de deuxi\`eme g\'en\'eration $X^{2nd}$  : \\
\hphantom{aaaaa}\texttt{ TDI : X2 ; } \\
...
\label{SSConfXML}
LISACode peut \'egalement ce configurer \`a partir d'un fichier XML.
%construit avec l'interface graphique \textbf{LISA\_AutoGUI.jar}
La structure du fichier XML est celle utilise dans le Mock LISA Data Challenge (cf. articles~\cite{MLDCHowTo,MLDC1Res,MLDC2} et site web~\cite{MLDCWeb}. 
...
\label{SSSErrInstall}
\begin{itemize}
\item V\'erifier que le syst\`eme poss\`ede bien une version de fftw3 et si n\'ecessaire sp\'ecifier son emplacement en moment de la configuration comme par exemple : \\ 
 {\tt CPPFLAGS=-I/cheminfftw3/include LDFLAGS=-L/cheminfftw3/lib}
...
\label{SSupport}
\subsection{A l'APC (AstroParticule et Cosmologie)}
\label{SSSupAPC}
Antoine PETITEAU : 
%petiteau@apc.univ-paris7.fr 

\subsection{A l'AEI (Albert Einstein Institute)}
\label{SSSupAEI}
