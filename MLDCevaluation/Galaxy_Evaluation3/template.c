...
void XYZ(double ***d, double f0, long q, long M, double *A, double *E);

void LISA_PREP(double *params, long *output, double Sn);
void FAST_LISA(double *params, long Np, long N, long M, double *A, double *E);

void template(double *params, long NP, double *hA, double *hE, long NX, long *hM, long *hp, double Sn)
{
  int i;
  long *output;
  long N, M, p;
  ...
  N = output[1];   M  = output[2];  p = output[3];
  
  //cap NS and MS to operate within range of FAST_LISA.
  if(M<N) M = N;
  if(M>NX) M = NX;
  
  //template generator 
  FAST_LISA(params, NP, N, M, hA, hE);
         
  //return central bin & template length
  *hp = p;
  *hM = M;
...
void LISA_PREP(double *params, long *output, double Sn)
{
  ...
  long M,N;
  long q;
  double f,Amp,theta,phi,psi,phi0,iota,fdot;
  double cosf, sinf, cos2f, sin2f;
  double fonfs;
  double BW,BS;
  double Ss, Sa, SN;
  double sqT;
  ...
  /*  Calculate constants related to GW source  */
  q = (int)floor(params[1]*T);
  
  /*  Calculate numeric constants  */
  sqT = sqrt(T); 

  fonfs = f/fstar;
  cosf = cos(fonfs);  cos2f = cos(2.0*fonfs);
  sinf = sin(fonfs);  sin2f = sin(2.0*fonfs);

  BW = 2.0*(2.0*pi*f*AU/c + 5.0);

  /*   Calculate noise   */
  //SN = AEnoise(f);

  /*   Determine necessary array sizes   */
  /*   N fixes the number of slow data points to calcualate in the 
       FAST_LISA subroutine   */
  ...
  /*   M fixes the number of fast terms to calculate in the 
       FAST_LISA subroutine   */
  /*   compare the amplitude to the noise in a bin
       this is used to calculate how far we need to follow the sinc */
  BS = Amp*sqT/(sqrt(Sn));
  
  M = (long)(pow(2.0,(rint(log(BS)/log(2.0))+1.0)));
  if(M < N) M = N;
  if(M > 8192) M = 8192;
  
 
  /*   Fill output array   */
  output[1] = N;  output[2] = M;  output[3] = q;  
...
void FAST_LISA(double *params, long Np, long N, long M, double *A, double *E)
{
  ...
  double **dplus, **dcross;
  /*   GW Source data   */
  double Mc, theta, phi, psi, D, iota, Amp, Aplus, Across, f0, fdot, phio;
...
{ 
  ...
  double phiLS, phiSL, cLS, sLS, cSL, sSL;
   ...
   for(i=1; i<=M; i++)
   {
     ...
     /*A[2*i-1] = sqT*((2.0*X[2*i-1]-Y[2*i-1]-Z[2*i-1])*cLS-(2.0*X[2*i]-Y[2*i]-Z[2*i])*sLS)/3.0;
     A[2*i]   = -sqT*((2.0*X[2*i-1]-Y[2*i-1]-Z[2*i-1])*sLS+(2.0*X[2*i]-Y[2*i]-Z[2*i])*cLS)/3.0;

     E[2*i-1] = sqT*((Z[2*i-1]-Y[2*i-1])*cLS-(Z[2*i]-Y[2*i])*sLS)/sq3;
     E[2*i]   = -sqT*((Z[2*i-1]-Y[2*i-1])*sLS+(Z[2*i]-Y[2*i])*cLS)/sq3;*/
     
     A[2*i-1] = sqT*(2.0*XLS[2*i-1] -YLS[2*i-1] - ZLS[2*i-1])/3.0;
     A[2*i]   = sqT*(2.0*XLS[2*i]   -YLS[2*i]   - ZLS[2*i])/3.0;

     E[2*i-1] = sqT*(ZLS[2*i-1] - YLS[2*i-1])/sq3;
     E[2*i]   = sqT*(ZLS[2*i]   - YLS[2*i])/sq3;
     
     /*A[2*i-1] = sqT*(2.0*XSL[2*i-1] -YSL[2*i-1] - ZSL[2*i-1])/3.0;
     A[2*i]   = sqT*(2.0*XSL[2*i]   -YSL[2*i]   - ZSL[2*i])/3.0;

     E[2*i-1] = sqT*(ZSL[2*i-1] - YSL[2*i-1])/sq3;
     E[2*i]   = sqT*(ZSL[2*i]   - YSL[2*i])/sq3;*/
