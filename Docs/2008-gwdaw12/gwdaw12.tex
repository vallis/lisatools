...
%PROBLEM: would be nice to put [rad] in the axis labels rather than here in the caption
%PROBLEM: would be nice to show error ellipse
\end{figure}
...
%PROBLEM: the SNRs reported are not what we define above.
%PROBLEM: these values are minus the parameter errors as defined above
%PROBLEM: parentheses maybe nicer than asterisks
...
%PROBLEM: may be more graceful to label corrected errors with asterisks and omit the row label
...
\end{tabular}
\end{table}
%PROBLEM: alignment of third column (could to the right)
...
%
\begin{table}
\caption{\label{Table_1b_1_4_correlations} Correlations, number of recovered sources, and number of false positives for Challenges 1B.1.2, 1B.1.4, 1B.1.5. Correlations computed after correcting extrinsic parameters using the ${\cal F}$-statistic are denoted with an asterisk (*).}
\begin{indented} \lineup
...
%PROBLEM: need GWDAW ref for BBGP and MT
...
%
\bibitem{rrv} Robinson E L, Romano J D and Vecchio A 2008 ``Search for a stochastic gravitational-wave signal in the second round of the Mock LISA Data Challenges'' in this volume
%
...
\br
Group & $C$ & \# recovered \\
\centre{3}{Challenge 1B.1.2}  \\
\centre{3}{${\rm SNR}_{\rm opt}=634.918$, $25$ sources}  \\
\mr
AEI		& $-0.822$	& 25	 	\\
GSFC		& \m0.006	& 25	 	\\
MCMNJU	& \m0.267	& 25	 	\\
